...
********************************************************************************
**                                                                            **
**  Class......:  clsSpectrum (Thread)                                        **
**                                                                            **
**  Description:  Creates thread to process a connection to a single Rockwell **
**                Spectrum Automated Call Distributor (ACD) using the Rockwell**
**                Transaction Link Protocol Version 6.1 (or above).           **
**                                                                            **
********************************************************************************
*******************************************************************************/
package jserver;
...
public class clsSpectrum extends Thread {
  ...
  private Vector vCalls;                                        // Call Vector - holds active calls
  private clsLogFile oLOG;                                      // Spectrum Log File Object
  private clsCTIServer oCTI;                                    // CTI Server
  ...
  public clsSpectrum(clsCTIServer oCTIServer, String sIP, int iPort) {
    ...
    oLOG = new clsLogFile("C:\\", "SPECTRUM", sVersion);

    this.oCTI = oCTIServer;

    oLOG.LogWrite(oLOG.LOG_INFO, sLogKey + "Call vector table initialized.");
    vCalls = new Vector();                                      // Instantiate the call vector
...
//            SubscriberInformation();
    ...
    //--- Release call table memory
    oLOG.LogWrite(oLOG.LOG_INFO, sLogKey + "Deleting Call Vector on normal shutdown...");
    vCalls.removeAllElements();
  ...
  *****************************************************************************/
  public void CallEstablished() {
    ...
    while (iByte <= iDataLenRecvd) {
      switch(iData[iByte]) {
        ...
        case CON_SPC_ELM_CALLING_PARTY_NUMBER: {				// The number of the person calling
          ...
          iType = iData[iByte+3];
          sLog += " ANI=" + sANI + " TYP=" + (char) iType;
          //sprintf(cLog, "Calling Party Number: pos=%d, len=%d, numberID=%d, type=%d, plan=%d, number=%s", iByte, iData[iByte + 1], iData[iByte + 2], iData[iByte + 3], iData[iByte + 4], cNumber);
          break;
        }
    ...
    //--- Log Call Data
    oLOG.LogWrite(oLOG.LOG_INFO, sLogKey + "[ESTABLISHED]" + sLog);
  ...
  **  Method.......: CallCleared                                              **
  **                                                                          **
  **  Description..: This method parses the Call Cleared Message from the     **
  **                 Rockwell Spectrum. This message indicates end of call.   **
  **                                                                          **
  **  Notes........: This implements the packet parser for Rockwell Trans-    **
  **                 action Link Version 6.1.                                 **
  **                                                                          **
  *****************************************************************************/
  ...
  /*****************************************************************************
  **                                                                          **
  **  Method.......: SubscriberInformation                                    **
  **                                                                          **
  **  Description..: This method parses the Subscriber Information message    **
  **                 that may/may not contain a more specific DNIS.           **
  **                                                                          **
  **  Notes........: This implements the packet parser for Rockwell Trans-    **
  **                 action Link Version 6.1.                                 **
  **                                                                          **
  *****************************************************************************/
  ...
  public void Update(long CallID, String ANI, String DNIS, int LWN, int Type) {
    ...
    bIsOn = oCTI.IsLwnOn(LWN);

    if (bIsOn) {                              // If the LWN is signed on to receive screen pops

      oLOG.LogWrite(oLOG.LOG_INFO, sLogKey + "Checking vectors...");

      if (vCalls.size() > 0) {                // If the size of the call vector is greater than 0
        for (int i=0;i<vCalls.size();i++) {   // Look for call in the vector
          clsCall c;                          // Pointer to call object
          c = (clsCall) vCalls.elementAt(i);  // Retrieve current call rec
          if (c.CallID == CallID) {           // If the IDs match, update the record
            c.Update(CallID, ANI, DNIS, LWN, Type);
            bFound = true;
          }
        }
      }
      if (!bFound) {                          // The Call is not in the vector

        oLOG.LogWrite(oLOG.LOG_INFO, sLogKey + "Adding to call vector...");

        clsCall oCall = new clsCall();        // Create a new call object

        oCall.Update(CallID, ANI, DNIS, LWN, Type);

        vCalls.addElement(oCall);
      }
    }
  ...
  //--- Remove a call object from the Vector
  public void Delete(long CallID) {
    int i=0;
    for (i=0;i<vCalls.size();i++) {
      clsCall c;
      c = (clsCall) vCalls.elementAt(i);
      if (c.CallID == CallID) {
        oCTI.ScreenCLEAR(c.CallID, c.LWN, c.DNIS, c.ANI);
        vCalls.removeElementAt(i);
        break;
      }
    }
  }

  //--- Analyze call vector and screen pop messages.
  public void ScreenPOP() {
    if (vCalls.size() > 0) {
      for (int i=0;i<vCalls.size();i++) {
        clsCall c;
        c = (clsCall) vCalls.elementAt(i);
        if ((c.CallID != 0) && (c.LWN !=0) && (c.DNIS != "")) {           // && (c.ANI != "") FOR ANI SCREEN POPS!
          //---- Perform the screen pop
          oCTI.ScreenPOP(c.CallID, c.LWN, c.DNIS, c.ANI);
          // vCalls.removeElementAt(i);
          break;
        }
      }
...
/*******************************************************************************
 **
 **  Class......:  clsCall (Object)
 **
 **  Description:  This simply defines an object for each active call.
 **
 ******************************************************************************/
class clsCall {
  public long CallID = 0L;        // Call ID from the Rockwell Spectrum
  public int Type = 0;            // Call Type
  public int LWN = 0;             // Terminating Logical Workstation Number
  public String ANI = "";         // ANI (Calling Party Number)
  public String DNIS = "";        // DNIS

  public clsCall() {
    ;
  }

  public void Update(long lCallID, String sANI, String sDNIS, int iLWN, int iType) {
    this.CallID = lCallID;
    if (this.ANI == "") this.ANI = sANI;
    if (this.Type == 0) this.Type = iType;
    if (this.LWN == 0) this.LWN = iLWN;
    this.DNIS = sDNIS;
  }
